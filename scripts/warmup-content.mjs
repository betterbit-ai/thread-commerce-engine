...
if (post.kind === 'affiliate') {
  ...
  if (
    !post.text.includes(disclosure) ||
    !post.link_url?.startsWith('https://link.coupang.com/') ||
    post.text.includes('https://link.coupang.com/')
  )
...
console.log(`warmup_link_url=${post.link_url ?? ''}`);
